...
<footer class="border-t border-line bg-white">
    <div class="mx-auto flex max-w-page flex-col gap-8 px-4 py-10 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:gap-5">
            <img src="<?= e(asset('images/khan-travel-logo.png')) ?>" alt="Khan Travel" width="128" height="128" loading="lazy" class="h-12 w-12 flex-none self-start sm:self-auto">
            <p class="text-sm leading-relaxed text-ink-muted">
                <span class="font-semibold text-ink">Khan Travel B2B Partner Program</span><br>
                Powered by TravelBookingPanel
            </p>
        </div>
...
<?php if ($privacyUrl !== ''): ?>
                <li><a class="hover:text-brand-600" href="<?= e($privacyUrl) ?>" target="_blank" rel="noopener">Privacy Policy</a></li>
<?php endif; ?>
<?php if ($mainSite !== ''): ?>
...
<?php endif; ?>
    ...
    <div class="border-t border-line">
        <p class="mx-auto max-w-page px-4 py-5 text-[13px] text-ink-muted sm:px-6 lg:px-8" style="padding-bottom: max(1.25rem, env(safe-area-inset-bottom, 0px))">
            &copy; <?= date('Y') ?> Khan Travel. All rights reserved.
        </p>
    </div>
</footer>
